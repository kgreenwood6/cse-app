...
# Obtain the genres and their length
...
def get_content_recommendations(book_ids):
    ...
    for book_id in book_ids:
        ...
        content_based_recs.extend(content_rows)
        ...
        wildcard_recs.extend(wildcard_rows)
    ...
    content_based_recs = random.sample(content_based_recs, 5)
    wildcard_rec_final = random.sample(wildcard_recs, 1)
    return content_based_recs, wildcard_rec_final


# Define a function to get the recommended book IDs from collaborative filtering
@st.cache_data
def get_collaborative_recommendations(book_ids):

    # Create an empty list to store the collaborative filtering recommendations
    collaborative_recs = []
    ...
    for book_id in book_ids:
        ...
        query = """SELECT user_id
                   ...
                   WHERE book_id = '%s' AND rating > 3""" % book_id

        cursor.execute(query)
        rows = cursor.fetchall()
        rows = [val[0] for val in rows]
        selected_users = random.sample(rows, 2)
        ...
        for user in selected_users:
            ...
            book_rows = [val[0] for val in book_rows if val[0] is not None]
            collaborative_recs.extend(book_rows)
...
collab_recs = get_collaborative_recommendations(book_ids=liked_book_ids)
final_recs_all = content_recs + collab_recs
final_recs_five = random.sample(final_recs_all, 5)


# Store the final recommended book IDs in the respective variables
recommended_books = get_book_details(book_ids=final_recs_five)
